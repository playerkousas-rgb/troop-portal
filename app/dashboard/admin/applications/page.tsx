    ...
    <main className="max-w-3xl mx-auto px-4 py-4 pb-24 space-y-4">
      ...
      {tab === 'pending' && (
        <div className="space-y-2">
          {pendingApps.length === 0 && <p className="text-center text-sm text-slate-400 py-8">沒有待批核的申請</p>}
          {pendingApps.map(a => (
            <div key={a.id} className="bg-white rounded-xl border border-slate-200 p-4">
              ...
              <div className="flex gap-2 mt-3">
                <button className="flex-1 text-[10px] font-bold py-2 rounded-lg bg-emerald-600 text-white">✅ 批核</button>
                <button className="flex-1 text-[10px] font-bold py-2 rounded-lg bg-rose-100 text-rose-700">❌ 拒絕</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {pendingSupplies.map(a => (
            <div key={a.id} className="bg-white rounded-xl border border-amber-200 bg-amber-50 p-4">
              ...
              <div className="text-[10px] text-slate-500">申請人：{a.requester} · {a.branch}</div>
              <div className="flex gap-2 mt-3">
                <button className="flex-1 text-[10px] font-bold py-2 rounded-lg bg-emerald-600 text-white">✅ 批核借用</button>
                <button className="flex-1 text-[10px] font-bold py-2 rounded-lg bg-rose-100 text-rose-700">❌ 拒絕</button>
              </div>
            </div>
          ))}
        ...
        <div className="space-y-2">
          {historyApps.map(a => (
            <div key={a.id} className="bg-white rounded-xl border border-slate-200 p-3 flex items-center justify-between opacity-70">
              <div className="flex items-center gap-2">
                <span className={`text-[8px] px-1.5 py-0.5 rounded font-bold ${a.status === 'approved' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                  {a.status === 'approved' ? '✅ 已批核' : '❌ 已拒絕'}
                </span>
                <span className="font-bold text-[11px]">{a.name}</span>
                <span className="text-[9px] text-slate-400">{a.branch}</span>
              </div>
              <span className="text-[9px] text-slate-400">{a.date}</span>
            </div>
          ))}
