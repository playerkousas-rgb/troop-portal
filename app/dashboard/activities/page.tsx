          ...
          {CROSS_BRANCH_ACTIVITIES.map(a => (
            <div key={a.id} className="bg-white rounded-xl p-2.5 border border-violet-100 mb-1.5">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[11px] bg-violet-100 text-violet-700 px-1.5 py-0.5 rounded font-bold mr-1">{a.branch}</span>
                  <span className="font-bold text-[11px]">{a.title}</span>
                </div>
                <span className="text-[11px] text-slate-500">{a.date}</span>
              </div>
              <div className="text-[11px] text-slate-500 mt-1">{a.location}</div>
              <button className="mt-1.5 w-full text-[11px] font-bold py-1.5 rounded-lg bg-violet-100 text-violet-700">❤️ 有興趣幫忙</button>
            </div>
          ))}
        </section>
      )}
                ...
                <div className="flex gap-1.5">
                  {role === 'parent' && a.myStatus !== 'registered' && (
                    <>
                      ...
                      <button className="flex-1 text-[11px] font-bold py-1.5 rounded-lg bg-pink-100 text-pink-700">❤️ 有興趣</button>
                  ...
                  {role === 'member' && (
                    <>
                      <button className="flex-1 text-[11px] font-bold py-1.5 rounded-lg bg-pink-100 text-pink-700">❤️ 有興趣</button>
                      <span className="text-[11px] text-slate-500 self-center">參加由家長確認</span>
                    </>
                  )}
                  {a.myStatus === 'registered' && a.myPaid !== 'confirmed' && (
                    <button className="text-[11px] font-bold py-1.5 px-3 rounded-lg bg-amber-100 text-amber-700 border border-amber-200">💰 標記已付款</button>
                  )}
                </div>
              </div>
            )}
      ...
      {isParentOrMember && (
        <section className="bg-white rounded-2xl border border-slate-200 p-4">
          <button onClick={() => setShowPast(!showPast)} className="w-full flex items-center justify-between text-left">
            ...
            <span className="text-slate-500 text-xs">{showPast ? '▲ 收起' : '▼ 展開'}</span>
          </button>
          {showPast && (
