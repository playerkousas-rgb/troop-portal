      ...
      <section className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl border border-amber-200 p-4">
        <div className="flex items-center justify-between mb-2">
          <h2 className="font-bold text-sm flex items-center gap-1.5">
            <span className="bg-amber-400 text-white text-[10px] font-extrabold px-2 py-0.5 rounded-lg">📣</span>
            最新消息
          </h2>
          {isLeader && (
            <button className="text-[10px] bg-amber-500 text-white px-2.5 py-1 rounded-lg font-bold">+ 發佈</button>
          )}
        </div>
            ...
            <div key={n.id} className={`rounded-xl px-3 py-2 flex items-center justify-between ${n.urgent ? 'bg-white border border-amber-200' : 'bg-white/60'}`}>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  {n.urgent && <span className="text-[8px] bg-rose-100 text-rose-700 px-1 py-0.5 rounded font-bold">急</span>}
                  <span className="font-bold text-[11px] truncate">{n.title}</span>
                </div>
                <div className="text-[9px] text-slate-400">{n.date} · {n.target}</div>
              </div>
            </div>
          ))}
        </div>
          ...
          {NOTICES.map(n => (
            <div key={n.id} className="bg-white rounded-xl border border-slate-200 p-3 flex items-center gap-3 card-hover">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[9px] text-slate-400">{n.date} · {n.branch}</div>
              </div>
              <span className="text-[9px] text-brand-600 font-bold flex-shrink-0">查看 →</span>
            </div>
          ))}
        </div>
      </section>
        ...
        <section className="bg-white rounded-2xl border border-slate-200 p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button className="bg-blue-600 text-white py-3 rounded-xl text-[11px] font-bold">
              📚 圖書館引入
              <div className="text-[9px] font-normal opacity-80 mt-0.5">外部通告接入</div>
            </button>
            <button className="bg-emerald-600 text-white py-3 rounded-xl text-[11px] font-bold">
              📝 自行上傳
              <div className="text-[9px] font-normal opacity-80 mt-0.5">Word 通告 + 標記資訊</div>
            </button>
          </div>
          {/* 手機上傳提示 */}
          <div className="bg-slate-50 rounded-xl p-2.5 flex items-center gap-2">
            <span className="text-sm">💻</span>
            <span className="text-[9px] text-slate-500">上傳文件建議使用電腦版，操作更方便。</span>
          </div>
        </section>
      )}
    </main>
