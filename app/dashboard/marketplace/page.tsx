...
export default function MarketplacePage() {
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-4 pb-24 space-y-4">
      ...
      <p className="text-[10px] text-slate-500">瀏覽及安裝擴充元件。Tier 2 即插即用，Tier 3 需要自設後端。</p>
          ...
          <div key={i} className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[8px] bg-violet-100 text-violet-700 px-1 py-0.5 rounded font-bold">Tier {p.tier}</span>
              </div>
              <p className="text-[10px] text-slate-500">{p.desc}</p>
            </div>
            <button className={`text-[10px] px-3 py-1.5 rounded-lg font-bold ${p.installed ? 'bg-slate-100 text-slate-500' : 'bg-brand-600 text-white'}`}>
              {p.installed ? '已安裝' : '安裝'}
            </button>
          </div>
        ))}
