    ...
    <div className="space-y-4">
      ...
      <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
        <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
          ...
          <span className="text-[11px] text-slate-500 font-bold">共 {troops.length} 個旅團</span>
        </div>

        {troops.length === 0 ? (
          <p className="text-[11px] text-slate-500 m-0">暫無已接入旅團，歡迎按下方步骤申請接入。</p>
        ) : (
            ...
            {troops.map(t => {
              ...
              return (
                <div key={t.key} className="rounded-xl border border-slate-200 bg-slate-50/60 p-3.5">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-[11px] text-slate-500 font-semibold">編號 {t.id}</div>
                      </div>
                    </div>
                    <span
                  ...
                  {t.note && <p className="text-[11px] text-slate-500 mt-2 mb-0 leading-relaxed">{t.note}</p>}
                  <Link
                    href="/"
                    className="inline-flex mt-2.5 text-[11px] font-bold text-brand-700 bg-brand-50 border border-brand-200 rounded-lg px-2.5 py-1.5 no-underline hover:bg-brand-100 transition"
                  >
                    到首頁選擇此旅團 →
                  </Link>
                </div>
      ...
      <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
        ...
        <p className="text-[11px] text-slate-500 leading-relaxed mt-0 mb-3">
          代表尚未開通。先照接入教學建立 Google Sheet 後台，再提交接入申請；管理員審核通過後就會加入上面的登記表。
        </p>
        <div className="flex gap-2 flex-wrap">
          <Link
            href="/setup"
            className="no-underline text-[11px] font-bold bg-white text-slate-700 border border-slate-200 px-3 py-2 rounded-xl hover:bg-slate-50 transition"
          >
            📖 接入教學
          </Link>
          <Link
            href="/onboard"
            className="no-underline text-[11px] font-bold bg-brand-600 text-white px-3 py-2 rounded-xl hover:bg-brand-700 transition"
          >
            📨 提交接入申請
          </Link>
          <Link
